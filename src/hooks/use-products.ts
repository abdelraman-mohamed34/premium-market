...
export function useCreateProduct(tenantId: string) { const qc = useQueryClient(); return useMutation({ mutationFn: createProductAction, onSuccess: () => qc.invalidateQueries({ queryKey: productKeys.list(tenantId) }) }); }
