...
export function useStoreProducts(tenantSlug: string | null, tenantId: string | null) {
    ...
    const products: UiProduct[] = isSandbox ? mockProducts : (live.data ?? []).map((product) => ({ id: Number(product.id), tenant_id: product.tenant_id, title: product.title, price: `EGP ${product.price.toFixed(2)}`, originalPrice: product.compareAtPrice ? `EGP ${product.compareAtPrice.toFixed(2)}` : undefined, image: product.images, colors: product.colors, tags: product.tags, rating: product.rating, reviewsCount: product.reviewsCount, reviews: product.reviews.map(review => ({ id: Number(review.id), author: review.author, rating: review.rating, comment: review.comment, date: String(review.date) })) }));
